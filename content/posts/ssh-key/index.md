...
description = '一份只有兩行的白名單，人跟 AI 共用的兩把鑰匙——關於信任、撤回、與裝置入口的故事。'
...
於是登入過程像一場「考試」：NAS 出題（給你一段隨機文字）、你的設備用私鑰算出答案、NAS 用公鑰驗答案對不對。

**整段你的私鑰沒離開過你的設備** ＝「值得偷走」的東西根本沒上網。
...
真正會危險的不是這兩關有漏洞，而是「**你的私鑰被偷**」（不是被試出來，而是被拿走）。所以好習慣是：
